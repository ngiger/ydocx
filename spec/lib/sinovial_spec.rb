...
describe "ydocx" do
  ...
  it "should convert sinovial_FR to xml" do
    ...
    File.exist?(sinovial_FR).should be true
    ...
    doc.output_file("xml")
    File.exist?(sinovial_FR_xml).should be true
    doc.parser.lang.to_s.should
    doc = Nokogiri::XML(File.open(sinovial_FR_xml))
    doc.xpath('//chapters/chapter[contains(heading, "Fabricant")]').size.should
    doc.xpath('//chapters/chapter[contains(heading, "Distributeur")]').size.should
    doc.xpath('//chapters/chapter[contains(heading, "Remarques particuli")]').size.should > 0
  end

  it "should convert sinovial_DE to xml" do
    sinovial_DE = File.join(YDcoxHelper::DataDir, "Sinovial_DE.docx")
    File.exist?(sinovial_DE).should be true
    doc = YDocx::Document.open(sinovial_DE)
    sinovial_DE_xml = sinovial_DE.sub(".docx", ".xml")
    doc.to_xml(sinovial_DE_xml, {format: :fachinfo})
    doc.output_file("xml")
    File.exist?(sinovial_DE_xml).should be true
    doc.parser.lang.to_s.should
    doc = Nokogiri::XML(File.open(sinovial_DE_xml))
    doc.xpath('//chapters/chapter[contains(heading, "Packung")]').size.should
    doc.xpath('//chapters/chapter[contains(heading, "Hersteller")]').size.should
  ...
  it "should convert sinovial_DE to html" do
    ...
    doc.output_file("html")
    File.exist?(sinovial_DE_html).should be true
  end
