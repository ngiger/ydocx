...
module YDocx
  class Parser
    ...
    def parse_title(node, text)
      if @indecies.empty? and !text.empty? and node.previous and
        ...
        markup(:h1, text, {id: title.downcase})
      end
    end
  ...
  class Document
    ...
    def output_directory
      unless @files
        files = if @parser.code
          @directory + "/" + @parser.code
        else
          @path.basename(".docx").to_s + "_files"
        end
    ...
    def output_file(ext) # html
      ...
      if @parser.code
        ...
        output_directory.join "#{lang}_#{filename}.#{ext}"
      else # default
        @path.sub_ext(".#{ext}")
      end
    end
    ...
    alias_method :copy_or_convert, :organize_image
    ...
    def prepare_reference
      ARGV.reverse_each do |arg|
        if /\.(jpeg|jpg|png|gif)$/.match?(arg.downcase)
